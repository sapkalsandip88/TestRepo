			...
			<div class="row my-lg-5 mb-5">
				...
				<div class="side-bar col-lg-3">
					...
					<div class="search-hotel">
						<h3 class="shopf-sear-headits-sear-head">Test in focus</h3>
						<form class="nav-search form-inline my-0 form-control"
							...
							<select class="form-control input-lg" name="category"
								...
								<c:forEach var="alltestList" items="${alltestList}">
									<option value="${alltestList.testname}">${alltestList.testname}
										(${alltestList.altname})</option>
								</c:forEach>
								...
								<div class="card product-men p-3">
									<div class="men-thumb-item">
										<div class="men-cart-pro">
											<div class="inner-men-cart-pro">
												<a href="/quickViewT/${alltestList.testname}"
									...
									<div class="card-body  py-3 px-2">
										<h5 class="card-title text-capitalize">
											<b>${alltestList.testname}</b></br> ${alltestList.altname}
										</h5>
										<h5 class="card-title text-capitalize">${alltestList.dscr}</h5>
										<div class="card-text d-flex justify-content-between">
											<p class="text-dark font-weight-bold">&#8377;${alltestList.price}</p>
											<p class="line-through">&#8377;${alltestList.actprice}</p>
										</div>
									</div>
									<!-- card footer -->
									...
									<form action="/quickViewT" method="post">
										...
										<form action="#" method="post">
											<input type="hidden" name="cmd" value="_cart"> <input
												...
												value="${alltestList.testname}"> <input
												type="hidden" name="amount" value="${alltestList.price}">
											<button type="submit" class="hub-cart phub-cart btn">
						...
						<span>
							<center>
								<c:if test="${currentpage != 1}">
									<a href="/testWithDiscount/1/${discount}"> First</a>
								</c:if>
								<c:if test="${currentpage == 1}"> First</c:if>

								<c:if test="${currentpage > 1}">
									<a href="/testWithDiscount/${currentpage - 1}/${discount}"> <i
										class="far fa-caret-square-left"></i></a>
								</c:if>
								<c:forEach var="i" items="${totalpages}">
									<c:if test="${i != currentpage}">
										<a href="/testWithDiscount/${i}/${discount}">${i} </a>
									</c:if>
									<c:if test="${i == currentpage}">${i} </c:if>
								</c:forEach>
								<c:if test="${currentpage <lastpage}/${discount}">
									<a href="/testWithDiscount/${currentpage + 1}/${discount}"><i
										class="far fa-caret-square-right"></i></a>
								</c:if>
								<c:if test="${currentpage <lastpage}">
									<a href="/testWithDiscount/${lastpage}"> Last</a>
								</c:if>
							</center>
						</span>
